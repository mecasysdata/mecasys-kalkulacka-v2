...
def load_csv(url):
    try:
        ...
        # Necháme stĺpce tak ako sú, len odstránime biele znaky
...
df_c, df_m, df_z = load_csv(URLS["cisnik"]), load_csv(URLS["mat_cena"]), load_csv(URLS["zakaznici"])
...
class PDF(FPDF):
    def header(self):
        ...
        self.cell(0, 10, 'MECASYS - CENOVÁ PONUKA', 0, 1, 'L')
        self.ln(10)

def get_hustota(mat, ako, df_cis):
    mat = str(mat).upper()
    if 'NEREZ' in mat: return 8000.0
    if 'OCEĽ' in mat or 'OCEL' in mat: return 7900.0
    if 'FAREBNÉ' in mat or 'KOVY' in mat:
        if str(ako).startswith('3.7'): return 4500.0
        if str(ako).startswith('3.'): return 2900.0
        if str(ako).startswith('2.'): return 9000.0
    return 7850.0
...
with st.container(border=True):
    ...
    # OPRAVA: Dynamické načítanie stĺpca 'zakaznik'
    z_col = 'zakaznik' if 'zakaznik' in df_z.columns else df_z.columns[0]
    v_zakaznik = h3.selectbox("Zákazník", sorted(df_z[z_col].unique()) if not df_z.empty else ["-"])

with st.container(border=True):
    st.subheader("📦 Nová položka")
    c1, c2, c3 = st.columns(3)
    with c1:
        v_item_nazov = st.text_input("ITEM (Označenie / Výkres)", placeholder="napr. Hriadeľ 001")
        v_ks = st.number_input("Počet kusov", 1, value=10)
        v_nar = st.selectbox("Náročnosť (1-5)", ['1','2','3','4','5'], index=2)
    with c2:
        v_mat = st.selectbox("Materiál", sorted(df_c['MATERIAL'].unique()) if 'MATERIAL' in df_c.columns else [])
    ...
    with c3:
        ...
        v_n_koop_manual = st.number_input("Náklad kooperácia [€/ks]", 0.0) if v_ma_koop else 0.0

# --- 4. VÝPOČET ---
if st.button("➕ PRIDAŤ DO PONUKY", use_container_width=True):
    if not v_item_nazov:
        st.warning("Zadajte označenie ITEMu!")
    else:
        try:
            # Načítanie dát zákazníka s ohľadom na tvoje názvy stĺpcov
            z_row = df_z[df_z[z_col] == v_zakaznik]
            v_krajina = str(z_row['krajina'].iloc[0]) if 'krajina' in z_row.columns else "SK"
            v_lojalita = float(z_row['lojalita'].iloc[0]) if 'lojalita' in z_row.columns else 5.0
            ...
            st.session_state.items_cp.append({
                ...
                "lojalita": v_lojalita, "item": v_item_nazov, "material": v_mat, "akost": v_ako, "d": v_d, "l": v_l,
...
if st.session_state.items_cp:
    ...
    st.table(df_basket[["item", "ks", "jednotkova_cena", "cena_spolu"]])
    ...
    with c1:
        if st.button("📄 PDF"):
            ...
            for i in st.session_state.items_cp:
                pdf.cell(0, 10, f"{i['item']} - {i['ks']}ks - {i['cena_spolu']} EUR", 1, 1)
            st.download_button("Stiahnuť PDF", data=pdf.output(dest='S').encode('latin-1'), file_name="CP.pdf")
    with c2:
        if st.button("💾 ARCHÍV"):
            r = requests.post(URL_APPS_SCRIPT, json=st.session_state.items_cp)
            if "Success" in r.text:
                st.success("Uložené!"); st.session_state.items_cp = []; st.rerun()
    with c3:
